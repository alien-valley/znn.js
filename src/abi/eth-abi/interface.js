...
var Interface = /** @class */ (function () {
    ...
    Interface.prototype.getFunction = function (nameOrSignatureOrSighash) {
        ...
        if (nameOrSignatureOrSighash.indexOf('(') === -1) {
            var name_1 = nameOrSignatureOrSighash.trim();
            var matching = Object.keys(this.functions).filter(function (f) { return f.split('(' /* fix:) */)[0] === name_1; });
            if (matching.length === 0) {
                logger.throwArgumentError('no matching function', 'name', name_1);
            }
            else if (matching.length > 1) {
                logger.throwArgumentError('multiple matching functions', 'name', name_1);
            }
    ...
    Interface.prototype.getEvent = function (nameOrSignatureOrTopic) {
        ...
        if (nameOrSignatureOrTopic.indexOf('(') === -1) {
            ...
            var matching = Object.keys(this.events).filter(function (f) { return f.split('(' /* fix:) */)[0] === name_2; });
            if (matching.length === 0) {
                logger.throwArgumentError('no matching event', 'name', name_2);
            }
            else if (matching.length > 1) {
                logger.throwArgumentError('multiple matching events', 'name', name_2);
            }
    ...
    Interface.prototype.getError = function (nameOrSignatureOrSighash) {
        ...
        // It is a bare name, look up the function (will return null if ambiguous)
        if (nameOrSignatureOrSighash.indexOf('(') === -1) {
            var name_3 = nameOrSignatureOrSighash.trim();
            var matching = Object.keys(this.errors).filter(function (f) { return f.split('(' /* fix:) */)[0] === name_3; });
            if (matching.length === 0) {
                logger.throwArgumentError('no matching error', 'name', name_3);
            }
            else if (matching.length > 1) {
                logger.throwArgumentError('multiple matching errors', 'name', name_3);
            }
    ...
    Interface.prototype.getSighash = function (functionFragment) {
        ...
        return (0, properties_1.getStatic)(this.constructor, 'getSighash')(functionFragment);
    };
            ...
            if (param.name && result[param.name] == null) {
                ...
                if (value_1 instanceof Error) {
                    Object.defineProperty(result, param.name, {
                        get: function () {
                            throw wrapAccessError("property ".concat(JSON.stringify(param.name)), value_1);
                        }
                    });
                }
                else {
